package org.performance.simplethreaddumpparser;
...
public class ThreadDumpParser {
    public static void main(String[] args) {
        ...
        // Define the simplified regex pattern to extract information
        String regex = "\"([^\"]*)\" #(\\d+) (\\w+) prio=(\\d+).*cpu=([\\d.]+\\w+).*elapsed=([\\d.]+\\w+).*tid=(0x[0-9a-f]+).*nid=(0x[0-9a-f]+).*";
        ...
        if (matcher.find()) {
            ...
            System.out.println("CPU Time: " + matcher.group(5));
            System.out.println("Elapsed Time: " + matcher.group(6));
            System.out.println("Thread ID: " + matcher.group(7));
            System.out.println("Node ID: " + matcher.group(8));
        }
    }
}
